...
for language=config_feats.languages
    % Training data will be processed, otherwise continue with test data
    if config_feats.train
        % identify method for feature extraction.
        if strcmp(config_feats.method.name, 'mfcc')
            % Extract MFCCs (CMVN but no deltas)
            F_train = getMFCCs(ZS.(language{1}).train.filename,1, ...
                'white', 1000,config_feats.window_length, ...
                config_feats.window_shift,1,0);
        elseif strcmp(config_feats.method.name, 'mel')
            F_train = getMelSpectrogram(...
                ZS.(language{1}).train.filename, ...
        ...
        % Sample size
        seqlen = round(config_feats.sample_length / ...
            config_feats.window_shift);
        ...
        if config_feats.max_train
            % NOTE:
            % Hack to cut training data: allow max 2 GB training data, 
            % because scipy cannot read larger than that .mat files. This 
            % doesn't cut data on Mandarin, but does so on English.
        ...
        % For predictive modeling, create also temporally shifted versions 
        % of the data: short shift (50 ms, medium shift, 150 ms, and long 
        % shift, 400 ms).
        shift1 = 5; %
        shift2 = 15;
        shift3 = 40;
        ...
        % Split features into sample_length(s) sequences and store in a 
        % tensor called X_in, that is of format [training_sample x time x 
        % feats_dim] (the original no-shift version, and the three shifted 
        % versions).
        ...
        save(fullfile(full_feats_path, 'train_in1.mat'), 'X_in1', '-v7.3');
        save(fullfile(full_feats_path, 'train_in2.mat'), 'X_in2', '-v7.3');
        save(fullfile(full_feats_path, 'train_in3.mat'), 'X_in3', '-v7.3');
    ...
    if config_feats.test
        for duration=config_feats.durations
            ...
            % In case training data was not processed, we need to calculate
            ...
            for k = 1:length(F_test)
                ...
                % store signal ID
                [~,fil,~] = fileparts(ZS.(language{1}).test.(...
                    ['filename_' duration{1}]){k});
                ...
                if config_feats.reset_samples
                    % introduce reset. -1 id for file as we should ignore 
                    % those reset samples.
                    if k < length(F_test)
                        F_test_all(wloc:wloc+reset_size-1,:) = ...
                            reset_sample;
            ...
            % We need to generate all the test files, then, we need to pad 
            % last file if the test set is not mod 0 of the sample size.
            ...
            save(fullfile(full_feats_path, ...
                ['test_', duration{1} 's.mat']), 'X_test_in', ...
                'X_test_ind', '-v7.3');
        end
    end
